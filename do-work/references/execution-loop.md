...
2. **Read, in order** -- the issue (`gh issue view <N>`: Goal, What to build, Acceptance criteria, Test plan, and the `make-issues:meta` block); the TDD capability the `trace_tdd` IDs point at (in `docs/tdd-data.yaml`, and the TDD Markdown alongside it in `docs/` -- do not re-derive it); the `trace_prd` requirement for the why. Honor any binding constraints (`BC-`) the capability is `bounded_by`. If the issue cannot be built as written, escalate (below) instead of coding around it. **Under `--dangerously`** the worker does not escalate or wait: it resolves implementation gaps with best-practice defaults, mocks missing externals (`FIXME(dangerously)`), builds HITL too, and opens a `needs-human-review` follow-up for each decision -- see `escalation-and-handback.md`, *Dangerous mode*. It still never edits the PRD/TDD/scope.
