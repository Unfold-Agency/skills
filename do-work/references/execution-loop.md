...
2. **Select** (`select_work.py --autonomy afk`). This is the actionable queue: open, unflagged, unblocked (and not blocked by a stale-flagged blocker), autonomy-matched, not in flight elsewhere -- resumable-by-you first, then by `priority` (a live hook; make-issues does not stamp one today, so the default is by issue number), then issue number.
