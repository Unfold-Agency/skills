...
const workerPrompt = (item, dangerous) =>
  ...
  `AS-BUILT LEDGER (required in every verdict). For EACH acceptance criterion the issue carries, record one entry in as_built: {criterion: "<text or id>", status: "met" | "deferred" | "mocked", evidence: "<how you verified it, or why it is deferred/mocked>"}. Be truthful from what you ACTUALLY implemented: "met" needs real evidence (a passing test, a checked behavior) -- when the issue embeds a "## Verification" proof plan, CITE the entry you executed (e.g. "test (negative): <check> -- passing"), not a bare "works". "mocked" means it is backed by a FIXME(dangerously) stub. "deferred" means it was not built -- including a criterion whose only proof is demo/inspection: record the demo/inspection steps as the evidence and let it park; that row is the human reviewer's checklist, never something you claim. The orchestrator gates on this -- an honest "deferred"/"mocked" is right; a "met" that was not really met is a defect.\n\n` +
  (dangerous
