...
The planning layer is a layered spec set, not a PRD+TDD pair. Each spec is a single Markdown file whose **YAML frontmatter** carries the structured contract described below (make-spec emits one file per document -- there is no separate `*-data.yaml`); `arch-data.yaml` is make-arch's own plain-YAML file:
...
1. **Read the specs.** `overview.md` (goals, scope, the feature_index, optional `phasing`), every `features/*.md` (the requirements you will slice), and `arch-data.yaml` (the ADR index, for `trace_adr` and the embedded one-line decision snippets). Read `references/slicing-and-review.md` now.
