...
- `assets/labels.yaml` -- the label scheme (provenance, autonomy, `trace:`, `src:`) and the meta-block contract. Read it before ensuring labels or writing the meta block.
...
2. **Build the maps:** issue-by-capability (match on the meta block's `trace_tdd`, fall back to the `trace:<CAP>` label) and capability-to-fingerprint (`item_fingerprint.py`).
