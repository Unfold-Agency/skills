...
1. **`docs/tdd-data.yaml`** -- the capabilities you will slice: entities (`ENT-`), workflows (`WF-`), state machines (`STM-`), integrations (`INTG-`), NFRs (`TNF-`), and decisions (`ADR-`). Also `traceability.mapped` (which PRD requirement each capability satisfies) and the binding constraints (`BC-`) the work must respect.
2. **`docs/prd-data.yaml`** -- the objectives and success criteria each capability ultimately serves. The issue's *Goal* and *Success criteria* come from here, in the user's terms.
...
| `fingerprint` | `scripts/item_fingerprint.py docs/tdd-data.yaml --id <CAP>` |
...
**Place each item in its phase (when the TDD has a plan).** If the TDD declares `implementation_phases`, every issue rolls up to the phase its capability belongs to -- the grouping `do-work --phase=N` drains. Get the mapping once: `python scripts/phase_milestones.py docs/tdd-data.yaml --json` returns `cap_to_phase` (capability -> phase number) and `phase_title` (number -> milestone title `Phase <N>: <name>`). An item's phase is the phase of its `trace_tdd` capability; an item whose caps span phases takes the **latest** and is a slicing smell to flag and probably re-slice (`--trace ID,ID` reports it). Phase is **sequencing, not contract** -- it is carried by the GitHub **milestone**, never written into the issue body or meta block, so re-phasing later never churns the fingerprint. A TDD with no plan skips milestones entirely; nothing else about slicing changes.
...
- The work items, **grouped by phase** when the TDD has a plan (a phase header per `Phase N: name`, its items beneath). For each item: **title · AFK/HITL · blocked-by · trace IDs**.
- The **coverage check**: every `active` capability has at least one item, or the gap is named. Every PRD `must`/`should` requirement traces through to an item, or it is listed as unmapped. With a plan, also: every active phase has at least one item, every item maps to a phase, and any phase-spanning item is flagged.
- For a sync, the **reconciliation plan**: what will be created, updated, flagged, closed, or followed-up (see `reconciliation.md`), plus any milestone re-assignments.

Iterate until the human approves. Then, when the TDD has a plan, **ensure the milestones exist first** -- `python scripts/phase_milestones.py docs/tdd-data.yaml --ensure --repo <owner/name>` creates one milestone per phase (idempotent). Now create issues -- in dependency order, blockers first, so each `--blocked-by` references an issue number that already exists. Stamp every issue per the template, apply its labels (just `make-issues` and `afk`/`hitl`), and assign its phase milestone: `gh issue create ... --milestone "Phase <N>: <name>"`. Traceability and source versions are carried in the body (`## Traceability` table) and the meta block; the phase is carried by the milestone -- neither is a label.
