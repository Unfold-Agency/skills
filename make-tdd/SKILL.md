...
- `assets/tdd-data-schema.yaml` -- the schema for the derived data file, including the ID prefixes, the `meta` system-of-record fields, the optional `implementation_phases` collection, and validator rules V-001 through V-018. Read it before deriving the YAML.
...
- `scripts/validate_tdd.py` -- run after every YAML derivation: `python scripts/validate_tdd.py docs/tdd-data.yaml [--prd docs/prd-data.yaml] [--tdd-md docs/TDD-<project>.md] [--prev docs/archive/tdd-data-v<old>.yaml]`. Requires PyYAML (`pip install pyyaml --break-system-packages` if missing). Exit code 0 = pass. Never present a TDD whose data file fails validation; fix or downgrade items (an unsupported "requirement" becomes an assumption or an open question) until it passes, and tell the user about anything you downgraded.
...
4. **Propose the implementation plan.** Once the capabilities and their dependencies are settled, cluster the active ones into a handful of ordered, shippable phases (by dependency and risk) and present the plan recommendation-first for approval or changes -- describe each phase (what it covers, what it delivers, exit criteria, which phases it depends on), not just a list of IDs. This is recommended on every generate. Record the approved phases in Section 14 and `implementation_phases`; every active capability lands in exactly one phase (V-018). See `references/intake-and-questioning.md` (§5). It is what `make-issues` turns into one GitHub milestone per phase and what `do-work --phase=N` drains.
5. **Write the Markdown.** Fill the template. Embed diagrams as Mermaid (ERD, state machines, sequence flows), each with a failure branch where one exists. Strip all HTML guidance comments. Default NFRs (accessibility, security, observability) stay in unless explicitly waived -- record any waiver in the changelog.
6. **Derive and validate.** Generate `tdd-data.yaml` **in chunks** -- write `meta` first, then one record collection at a time (entities, then state machines, then workflows, then integrations, and so on, ending with `implementation_phases`), appending each. Do not emit the whole file in a single step; on a large design that times out and blows the context window. Then stamp `meta.fingerprint`, run `validate_tdd.py` with `--prd` pointed at `docs/prd-data.yaml` and `--tdd-md` at `docs/TDD-<project>.md`, fix failures, and report: capability count, requirement coverage (mapped vs. unmapped PRD IDs), the phase breakdown (how many phases and what each covers), binding-constraint count, open assumption count, open risk count, and anything you routed somewhere the user might not expect.
...
3. **Apply the diff.** Never renumber or delete IDs -- supersede or defer them. Record the change in the changelog with class, changed IDs, and trigger reference. If the TDD has an implementation plan, keep it covering: place any new active capability in a phase, drop superseded/deferred ones, and re-present the plan if the phasing changes materially. Re-sequencing a capability is a planning change, not a contract change -- `make-issues` re-aligns the milestone on the next sync without flagging the issue.
4. **Snapshot, re-derive (in chunks), and validate.** First copy the outgoing live files into `docs/archive/` (`docs/archive/TDD-<project>-v<old>.md` and `docs/archive/tdd-data-v<old>.yaml`). Then re-derive `docs/tdd-data.yaml` the same chunked way (meta, then one collection at a time), re-lock `meta.prd_version` to the PRD version this amendment satisfies, and regenerate the fingerprint. Validate with `--prev docs/archive/tdd-data-v<old>.yaml` (enforces the no-vanishing-IDs rule, V-009), `--prd docs/prd-data.yaml` (enforces coverage and the version lock, V-017), and `--tdd-md docs/TDD-<project>.md`.
...
**Repo layout & archiving.** When the system of record is the repo, the canonical pair is `docs/TDD-<project>.md` + `docs/tdd-data.yaml` (set `meta.repo.path` to the Markdown). The live files keep these names; the version lives in the frontmatter, never the filename. On every version bump, *before* applying the diff in amend mode, snapshot the outgoing pair into `docs/archive/` with the version in the name (`docs/archive/TDD-<project>-v<old>.md`, `docs/archive/tdd-data-v<old>.yaml`) -- the archived prior `tdd-data.yaml` is what the amend validator's `--prev` reads. (In a Claude Project, the pair lives in the Project; `docs/` is the repo layout, set at the promotion event below.)
