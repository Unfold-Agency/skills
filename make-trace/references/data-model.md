...
| Architecture | `docs/product/architecture.md` frontmatter (components, integrations) + `decisions/ADR-*.md` frontmatter (legacy fallback: `arch-data.yaml`) | `C-NNN` (components), `ADR-NNNN` (decisions), `INTG-<norm-name>` (integrations) | `C` / `ADR` / `INTG` |
| Issues | `gh issue list --state all` | `#N` | `issue` |

Integrations have no id of their own, so a stable `INTG-<normalized-name>`
...
- **architecture -> requirement**:
  - **ADR -> requirement**: union of the ADR's `governs: [req ids]` (from its
    frontmatter) and every requirement whose `governed_by` names that ADR.
...
non-empty -> empty. Architecture is exempt when the layer is absent entirely (lite
